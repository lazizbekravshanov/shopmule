...
export function AIInsights() {
  ...
  const { data: insights = [], isLoading, refetch, isRefetching } = useQuery<Insight[]>({
  ...
  const totalVisible = insights.filter(i => !dismissedIds.has(i.id)).length
  const hasMore = totalVisible > 3
    ...
    <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden">
      ...
      <div className="px-5 py-4 border-b border-neutral-100 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-neutral-500">Data-driven recommendations</p>
          </div>
        </div>
        <button
          onClick={() => {
        ...
        >
          <RefreshCw className={cn('w-4 h-4 text-neutral-500', isRefetching && 'animate-spin')} />
        </button>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="px-5 py-8 text-center">
          ...
          <p className="text-xs text-neutral-500 mt-1">No actionable insights right now.</p>
        </div>
      ) : (
        /* Insights List */
        <div className="divide-y divide-neutral-100">
      ...
      {hasMore && (
        ...
        >
          {expanded ? 'Show less' : `Show ${totalVisible - 3} more insights`}
        </button>
      )}
    </div>
  )
